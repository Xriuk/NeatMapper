	...
	public class NewMapsGenericTests {
		...
		public void ShouldCheckButNotMapOpenGenericTypes() {
			// 1 parameter
			{
				// No constraints
				{
					Assert.IsTrue(_mapper.CanMapNew(typeof(IEnumerable<>), typeof(IList<>)));

					Assert.ThrowsException<MapNotFoundException>(() => _mapper.Map(new[] { "Test" }, typeof(IEnumerable<>), typeof(IList<>)));
				}
				...
				// Class constraint
				{
					...
					Assert.IsTrue(mapper.CanMapNew(typeof(IEnumerable<>), typeof(int)));

					Assert.ThrowsException<MapNotFoundException>(() => mapper.Map(new[] { new Product() }, typeof(IEnumerable<>), typeof(int)));
				}
			}

			// 2 parameters
			{
				Assert.IsTrue(_mapper.CanMapNew(typeof(Tuple<,>), typeof(ValueTuple<,>)));

				Assert.ThrowsException<MapNotFoundException>(() => _mapper.Map(new Tuple<string, int>("Test", 4), typeof(Tuple<,>), typeof(ValueTuple<,>)));
			}
			...
			// 3 parameters
			{
				...
				Assert.ThrowsException<MapNotFoundException>(() => _mapper.Map(new Tuple<string, int>("Test", 2), typeof(Tuple<,>), typeof(ValueTuple<,,>)));
			}
		}
		...
		public void ShouldCheckButNotMapSingleOpenGenericType() {
			// Generic source
			{
				Assert.IsTrue(_mapper.CanMapNew(typeof(IEnumerable<>), typeof(string)));

				Assert.ThrowsException<MapNotFoundException>(() => _mapper.Map(new[] { 1, 2 }, typeof(IEnumerable<>), typeof(string)));
			}

			// Generic destination
			{
				Assert.IsTrue(_mapper.CanMapNew(typeof(int), typeof(IList<>)));

				Assert.ThrowsException<MapNotFoundException>(() => _mapper.Map(3, typeof(int), typeof(IList<>)));
			}
		}
