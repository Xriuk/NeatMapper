	...
	public class AsyncMergeMapsGenericTests {
		...
		public async Task ShouldCheckButNotMapOpenGenericTypes() {
			// 1 parameter
			{
				// No constraints
				{
					Assert.IsTrue(_mapper.CanMapAsyncMerge(typeof(IEnumerable<>), typeof(IList<>)));

					var source = new[] { "Test" };
					var destination = new List<string>();
					await Assert.ThrowsExceptionAsync<MapNotFoundException>(() => _mapper.MapAsync(source, typeof(IEnumerable<>), destination, typeof(IList<>)));
				}
				...
				// Class constraint
				{
					...
					Assert.IsTrue(mapper.CanMapAsyncMerge(typeof(IEnumerable<>), typeof(int)));

					await Assert.ThrowsExceptionAsync<MapNotFoundException>(() => _mapper.MapAsync(new[] { new Product() }, typeof(IEnumerable<>), 0, typeof(int)));
				}
			}

			// 2 parameters
			{
				Assert.IsTrue(_mapper.CanMapAsyncMerge(typeof(Tuple<,>), typeof(ValueTuple<,>)));

				await Assert.ThrowsExceptionAsync<MapNotFoundException>(() => _mapper.MapAsync(new Tuple<string, int>("Test", 4), typeof(Tuple<,>), (0, "AAA"), typeof(ValueTuple<,>)));
			}
			...
			// 3 parameters
			{
				...
				await Assert.ThrowsExceptionAsync<MapNotFoundException>(() => _mapper.MapAsync(new Tuple<string, int>("Test", 2), typeof(Tuple<,>), ("AAA", 0, true), typeof(ValueTuple<,,>)));
			}
		}
		...
		public async Task ShouldCheckButNotMapSingleOpenGenericType() {
			// Generic source
			{
				Assert.IsTrue(_mapper.CanMapAsyncMerge(typeof(IEnumerable<>), typeof(string)));

				await Assert.ThrowsExceptionAsync<MapNotFoundException>(() => _mapper.MapAsync(new[] { 1, 2 }, typeof(IEnumerable<>), "a", typeof(string)));
			}

			// Generic destination
			{
				Assert.IsTrue(_mapper.CanMapAsyncMerge(typeof(int), typeof(IList<>)));

				await Assert.ThrowsExceptionAsync<MapNotFoundException>(() => _mapper.MapAsync(3, typeof(int), new List<string>(), typeof(IList<>)));
			}
		}
