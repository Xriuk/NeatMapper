...
namespace NeatMapper.Tests.AsyncMapping {
	...
	public class AsyncNullableMapperTests {
		...
		[TestMethod]
		public async Task ShouldCheckButNotMapOpenNullable() {
			{
				Assert.IsTrue(_mapper.CanMapAsyncNew(typeof(int), typeof(Nullable<>)));

				await Assert.ThrowsExceptionAsync<MapNotFoundException>(() => _mapper.MapAsync(1, typeof(int), typeof(Nullable<>)));
			}

			{
				Assert.IsTrue(_mapper.CanMapAsyncNew(typeof(Nullable<>), typeof(int)));

				await Assert.ThrowsExceptionAsync<MapNotFoundException>(() => _mapper.MapAsync(1, typeof(Nullable<>), typeof(int)));
			}

			{
				Assert.IsTrue(_mapper.CanMapAsyncNew(typeof(Nullable<>), typeof(Nullable<>)));

				await Assert.ThrowsExceptionAsync<MapNotFoundException>(() => _mapper.MapAsync(1, typeof(Nullable<>), typeof(Nullable<>)));
			}
		}
