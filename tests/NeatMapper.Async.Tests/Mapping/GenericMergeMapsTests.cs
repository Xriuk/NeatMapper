...
namespace NeatMapper.Tests.Mapping {
	...
	public class GenericMergeMapsTests {
		...
		public Task ShouldNotMapNotMatchingGenericTypes() {
			...
			return TestUtils.AssertMapNotFound(() => _mapper.MapAsync<IEnumerable<string>, IList<int>>(new[] { "Test" }, new List<int>()));
		}
		...
		public Task ShouldMapDeepGenerics() {
			...
			// Does not throw
			return _mapper.MapAsync<IDictionary<string, IDictionary<int, IList<bool>>>, IEnumerable<bool>>(objectToMap, new List<bool>());
		}

		[TestMethod]
		public Task ShouldNotMapNotMatchingDeepGenerics() {
			// Types should be the same
			return TestUtils.AssertMapNotFound(() => _mapper.MapAsync<IDictionary<string, IDictionary<int, IList<bool>>>, IEnumerable<float>>(new Dictionary<string, IDictionary<int, IList<bool>>>(), new List<float>()));
		}
		...
		public async Task ShouldMapCollectionsWithoutElementsComparer() {
			{
				var tuples = await _mapper.MapAsync(new[] { new Tuple<string, int>("Test1", 4), new Tuple<string, int>("Test2", 5) }, new List<ValueTuple<int, string>>());
			...
			{
				var tuples = await _mapper.MapAsync(new[] { new Tuple<string, int>("Test1", 4), new Tuple<string, int>("Test2", 5) }, new LinkedList<ValueTuple<int, string>>());
			...
			{
				var tuples = await _mapper.MapAsync(new[] { new Tuple<string, int>("Test1", 4), new Tuple<string, int>("Test2", 5) }, new CustomCollection<ValueTuple<int, string>>());
