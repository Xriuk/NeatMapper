		...
		public MapperConfiguration(MapperConfigurationOptions options) {
			...
			foreach(var type in options.MapTypes
				...
				.Where(t => t.IsClass && t.GetInterfaces().Any(i => 
					i.IsGenericType &&
					(i.GetGenericTypeDefinition() == typeof(INewMap<,>) || i.GetGenericTypeDefinition() == typeof(IMergeMap<,>))))) {
				...
				if(type.IsGenericTypeDefinition) {
					...
					foreach(var interf in interfaces) {
						...
						if(!typeArguments.All(t => interfaceOpenGenericArguments.Contains(t)))
							...
							var map = (interf.GetGenericTypeDefinition() == typeof(INewMap<,>) ? genericNewMaps : genericMergeMaps);
					...
					foreach(var interf in interfaces) {
						...
						(interf.GetGenericTypeDefinition() == typeof(INewMap<,>) ? newMaps : mergeMaps)
